...
Bluedot is Quadrant Information Security's Threat Intellegence database that Sagan can query.  In order to use
...
The will query all TCP/IP addresses found in a log line and query for `Malicious`, `Tor` and `Proxy`
address that are no older than one month old.  If the time is set to ``none``,  then any IoCs found
...
This option works in conjunction with the ``sagan.yaml`` ``dynamic_load`` configurations.  When a rule is 
...
If ``priority`` is set, it will over ride the ``classtype`` priority.
