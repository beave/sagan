...
int logzilla_db_connect( _SaganConfig *config ) {
...
if (!mysql_real_connect(mysql_logzilla, dbh, dbu, dbp, dbn, MYSQL_PORT, NULL, 0)) {
     sagan_log(config, 1, "[%s, line %d] MySQL Error %u: \"%s\"", __FILE__,  __LINE__, mysql_errno(mysql_logzilla), mysql_error(mysql_logzilla));
     }
...
char *logzilla_db_query ( _SaganConfig *config, char *sql ) {
...
//   removelockfile(config);
   sagan_log(config, 0, "[%s, line %d] PostgreSQL Error: %s", __FILE__, __LINE__, PQerrorMessage( psql_logzilla ));
   }
...
snprintf(escprg, sizeof(escprg), "%s", sql_escape(Event->config, Event->program, 1));
snprintf(escmsg, sizeof(escmsg), "%s", sql_escape(Event->config, Event->message, 1));
...
sql=sqltmp;
logzilla_db_query(Event->config, sql);
