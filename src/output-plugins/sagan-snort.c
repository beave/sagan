...
snprintf(sqltmp, sizeof(sqltmp), "SELECT sig_class_id from sig_class where sig_class_name='%s'", Event->class);
sql=sqltmp;
sqlout = db_query(sql); 
...
if ( sqlout == NULL ) {
   ...
   snprintf(sqltmp, sizeof(sqltmp), "INSERT INTO sig_class(sig_class_id, sig_class_name) VALUES (DEFAULT, '%s')", Event->class);
   sql=sqltmp;
   db_query(sql);
...
snprintf(sqltmp, sizeof(sqltmp), "SELECT sig_id FROM signature WHERE sig_name='%s' AND sig_rev=%s AND sig_sid=%s", Event->f_msg, Event->rev, Event->sid);
sql=sqltmp;
...
if ( sqlout == NULL ) {
 ...
 snprintf(sqltmp, sizeof(sqltmp), "INSERT INTO signature(sig_name, sig_class_id, sig_priority, sig_rev, sig_sid, sig_gid) VALUES ('%s', '%d', '%d', '%s', '%s', %lu )", Event->f_msg, sig_class_id, Event->pri, Event->rev, Event->sid, Event->generatorid );
   sql=sqltmp;
   db_query(sql);
...
snprintf(sqltmp, sizeof(sqltmp), "INSERT INTO event(sid, cid, signature, timestamp) VALUES ('%d', '%" PRIu64 "', '%d', '%s %s')", config->sensor_id, counters->cid, sig_sid, date, time );
sql=sqltmp;
db_query(sql); 
...
/* "tcp" */
if ( ipproto == 6 )  {
snprintf(sqltmp, sizeof(sqltmp), "INSERT INTO tcphdr VALUES ( '%d', '%" PRIu64 "', '%d', '%d', '0', '0', '0', '0', '0', '0', '0', '0'  )", config->sensor_id, counters->cid, Event->src_port, Event->dst_port  );
sql=sqltmp;
db_query(sql);
} 
...
if ( ipproto == 17 )  {
snprintf(sqltmp, sizeof(sqltmp), "INSERT INTO udphdr VALUES ( '%d', '%" PRIu64 "', '%d', '%d', '0', '0' )", config->sensor_id, counters->cid, Event->src_port, Event->dst_port  );
sql=sqltmp;
db_query(sql);
}
...
if ( ipproto == 1 ) { 
snprintf(sqltmp, sizeof(sqltmp), "INSERT INTO icmphdr VALUES ( '%d', '%" PRIu64 "', '8', '8', '0', '0', '0' )", config->sensor_id, counters->cid );
sql=sqltmp;
db_query(sql);
}
...
snprintf(sqltmp, sizeof(sqltmp), "INSERT INTO data(sid, cid, data_payload) VALUES ('%d', '%" PRIu64 "', '%s')", config->sensor_id, counters->cid, t_hex_data);
sql=sqltmp;
db_query(sql);
