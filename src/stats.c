...
//        Sagan_Log(NORMAL, "           Malformed                : h:%" PRIu64 "|f:%" PRIu64 "|p:%" PRIu64 "|l:%" PRIu64 "|T:%" PRIu64 "|d:%" PRIu64 "|T:%" PRIu64 "|P:%" PRIu64 "|M:%" PRIu64 "", counters->malformed_host, counters->malformed_facility, counters->malformed_priority, counters->malformed_level, counters->malformed_tag, counters->malformed_date, counters->malformed_time, counters->malformed_program, counters->malformed_message);
            ...
            Sagan_Log(NORMAL, "           GeoIP2 Hits:             : %" PRIu64 " (%.3f%%)", counters->geoip2_hit, CalcPct( counters->geoip2_hit, counters->sagantotal) );
            Sagan_Log(NORMAL, "           GeoIP2 Lookups:          : %" PRIu64 "", counters->geoip2_lookup);
            Sagan_Log(NORMAL, "           GeoIP2 Misses            : %" PRIu64 "", counters->geoip2_miss);
#endif
            ...
            if (config->bluedot_flag)
                {
                    ...
                    Sagan_Log(NORMAL, "");
                    Sagan_Log(NORMAL, "          IP addresses in cache         : %" PRIu64 " (%.3f%%)", counters->bluedot_ip_cache_count, CalcPct(counters->bluedot_ip_cache_count, config->bluedot_max_cache));
                    ...
                    Sagan_Log(NORMAL, "");
                    Sagan_Log(NORMAL, "          Hashes in cache               : %" PRIu64 " (%.3f%%)", counters->bluedot_hash_cache_count, CalcPct(counters->bluedot_hash_cache_count, config->bluedot_max_cache));
                    ...
                    Sagan_Log(NORMAL, "");
                    Sagan_Log(NORMAL, "          URLs in cache                 : %" PRIu64 " (%.3f%%)", counters->bluedot_url_cache_count, CalcPct(counters->bluedot_url_cache_count, config->bluedot_max_cache));
                    ...
                    Sagan_Log(NORMAL, "");
                    Sagan_Log(NORMAL, "          Filenames in cache            : %" PRIu64 " (%.3f%%)", counters->bluedot_filename_cache_count, CalcPct(counters->bluedot_filename_cache_count, config->bluedot_max_cache));
